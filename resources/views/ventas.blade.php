...
<div class="d-flex align-items-center mb-4">
    <h2 class="mb-0"><i class="bi bi-bag-check text-success"></i> Ventas Confirmadas</h2>
    <span class="badge bg-secondary ms-3 fs-6">{{ $ventas->count() }} pedidos</span>
</div>
        ...
        @foreach($ventas as $index => $venta)
            <div class="accordion-item">
                ...
                <h2 class="accordion-header" id="heading{{ $venta->id }}">
                    <button class="accordion-button {{ $index === 0 ? '' : 'collapsed' }} bg-white py-3" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $venta->id }}" aria-expanded="{{ $index === 0 ? 'true' : 'false' }}" aria-controls="collapse{{ $venta->id }}">
                        ...
                        <div class="row w-100 align-items-center text-dark">
                            ...
                            <div class="col-6 col-md-3">
                                ...
                                <span>{{ $venta->created_at->format('d/m/Y H:i') }}hs</span>
                            </div>
                            <div class="col-6 col-md-3 mt-2 mt-md-0">
                                <span class="text-muted small d-block">TOTAL</span>
                                <strong class="text-primary">${{ number_format($venta->total, 2, ',', '.') }}</strong>
                            </div>
                ...
                <div id="collapse{{ $venta->id }}" class="accordion-collapse collapse {{ $index === 0 ? 'show' : '' }}" aria-labelledby="heading{{ $venta->id }}" data-bs-parent="#accordionCompras">
                    <div class="accordion-body bg-light p-4">
                        <div class="row g-4">
                                ...
                                <div class="card border-0 shadow-sm">
                                    <div class="table-responsive">
                                        <table class="table align-middle mb-0 table-borderless">
                                            ...
                                            <tbody>
                                                @foreach($venta->detalles_compra as $detalle)
                            ...
                            <div class="col-lg-4">
                                <div class="card border-0 shadow-sm h-100 p-3">
                                    ...
                                    <p class="mb-2 small">
                                        ...
                                        <span class="text-muted">{{ str_replace('_', ' ', ucfirst($venta->metodo_pago)) }}</span>
                                    </p>

                                    <p class="mb-0 small">
                                        <strong>Método de Entrega:</strong> <br>
                                        @if($venta->retiro_sucursal)
                                        ...
                                        @else
                                            ...
                                            @if($venta->direccion)
                                                <small class="text-muted d-block mt-1">
                                                    {{ $venta->direccion->calle }} {{ $venta->direccion->altura }} <br>
                                                    {{ $venta->direccion->ciudad }}, {{ $venta->direccion->provincia }} <br>
                                                    <span class="fw-bold">CP:</span> {{ $venta->direccion->codigo_postal }}
                                                </small>
                                            @endif
                                        @endif
                                    </p>
                            ...
                            <div class="col-lg-4">
                                <div class="card border-0 shadow-sm h-100 p-3">
                                    <h6 class="fw-bold border-bottom pb-2 mb-3 text-secondary"><i class="bi bi-person"></i> Datos del Cliente</h6>
                                    
                                    <p class="mb-2 small">
                                        <strong>Nombre y Apellido:</strong> <br>
                                        <span class="text-muted">{{ $venta->user->name }} {{ $venta->user->apellido }}</span>
                                    </p>

                                    <p class="mb-2 small">
                                        <strong>Correo Electrónico:</strong> <br>
                                        <span class="text-muted">{{ $venta->user->email }}</span>
                                    </p>
